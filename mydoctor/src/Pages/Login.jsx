...
const Login = () => {
  ...
  return (
   ...
   <form className='min-h-[80vh] flex items-center'>
    ...
    <button className='bg-amber-600 text-black w-full py-2 rounded-md text-base'>{state === 'Sign Up' ? "Create Account": "Login"}</button>
    {
      state === "Sign Up" 
      ? <p > Already have an account ? <span onClick={()=>setState ('Login')} className='text-amber-600 underline cursor-pointer'>Login here</span> </p>
      : <p> Create a new account? <span onClick={()=>setState ('Sign Up')} className='text-amber-600 underline cursor-pointer'>click here</span> </p>
    }
    </div>
    </form>
    )
